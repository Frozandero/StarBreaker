...
| G-force ball | `Screen_Small_Radar2` / LOD0 cockpit | `HC_HUD_Ship_G_Force_Ball_Master` | `g_force_ball_master.png` | — | — | aspect 1.0 square; blank-at-rest FIXED 2026-06-14 (`bb_state_filter` sole-root `Instantiated` exemption — class-wide, also unblanked velocity ball/countermeasures). Correct content = DRAK ADV ball (G1a generic-vs-drak angle FALSIFIED+reverted). Open (DEFERRED, see [[gforce-hud-blank-fix]]): **G1b** square gauge LETTERBOXES vs centres the ball — owner `bb_layout/engine_parts/engine_01.part:156-178` (`useRaw` CONTAIN+centre). Cover-fit tried+REVERTED (`a6bdf106b`→`206da66c0`): origin-anchored cover is PERFECT for g-force but BREAKS the velocity ball (mirrored layout — its `card_BallArea` is at canvas-RIGHT), so a fixed crop anchor isn't generic; needs content-aware ball-card centring (2-pass) or the RTT ball-only-screen model. G2 colour, G3 vector after G1b |
