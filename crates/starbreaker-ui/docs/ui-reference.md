...
| Velocity num | `screen_flight_hud_left_upper` / LOD0 cockpit | `HC_HUD_Ship_Velocity_Num_Master` | `ship_velocity_num_master.png` | — | — | PARITY PASS 2026-06-15 (DRAK cutlass variant, two readouts stacked centred: "0m/s" / "0.0 G"). Was 100% BLANK at rest. LANDED: (1) at-rest content — both engine `NumberVariable`s (`flightController.forwardVelocity`, `flightcontroller.totalgforceamount`, +`ForwardBackSpeedGoal`) absent in static replay → placeholders ('-', 'G'); pinned =0 in the default-value registry (`1f25845d2`, AccelerationLimiterRatio precedent) → velocity "0", g-force "0.0 G" ✓. (2) layout — `base_CurrentVelocityNumber` is a flex Column(axisJustification/cross/item=Center) whose two `WidgetCard`s author non-zero Auto (64>1.0) on both axes; the flex sizer had no content-fit path for value>1.0 text-backed column children → they filled the canvas, overlapped, dumped glyphs top-left. Added main+cross-axis content-fit via `auto_text_intrinsic_main`, gated `axisJustification==Center` (no frozen screen has it — medical pins fill for non-zero column Auto, workflow §10) → cards shrink-wrap + Center stacks/centres them (`0cdf6d526`, visual+snapshot guards GREEN after `--lod 0` export). Colour Bright≈white faithful. **OPEN (deferred, root-caused): (a) velocity "m/s" unit** — `LocalizedSIUnitFromNumber.unitSuffix="Speed"` is unimplemented (`eval.rs:81` only does G/M/K magnitude prefixes); the enum→loc-key→"m/s" mapping is engine-internal, NOT in canvas data (no hard-coded "m/s"). **(b) font ~7× too small** — render text 1.9% of screen h vs ref 20.6%/16.2%; canvas designed 1920×1080 with the readout small+centred, ref shows it enlarged → the small physical screen renders a ZOOMED content-view sub-rect of the canvas (MFD-style, mfd_view.rs); the mesh→sub-rect zoom is undecoded. **(c) background** — ref has a brighter rounded-rect panel; render's `Screen Background` is an edge-lit dark vignette. **TRAP: broadening `ir_compose` `center_anchored_heading` (Heading1→any label_style) to centre the text REGRESSES medical-bed title/desc (they share the 0.5/0.5-anchor + 0/0-node pattern but stay LEFT) — centre via card cross-fit, NOT the text-draw rule (owner-caught).** aspect 1.24 fixed 2026-06-14 |
